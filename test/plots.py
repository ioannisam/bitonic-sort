...
with open(FILE_PATH, 'r') as file:
    for line in file:
        ...
        if line.startswith('WHEN p = '):
            ...
            curr_t_line = next(file).strip()
            curr_t = float(curr_t_line.split(': ')[1]) if curr_t_line.split(': ')[1] != 'NaN' else np.nan
            N = 2 ** (curr_p + curr_q)
            curr_nps = N / curr_t if not np.isnan(curr_t) else np.nan
